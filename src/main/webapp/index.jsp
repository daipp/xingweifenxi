	...
	$(function() {
		...
		index_tabsMenu = $('#index_tabsMenu').menu({
			onClick : function(item) {
				...
				var closeTabsTitle = [];
				var t = index_tabs.tabs('getTab', curTabTitle);
				var tabIndex = index_tabs.tabs('getTabIndex', t);
				//alert(tabIndex);
				
				$.each(allTabs, function() {
					var opt = $(this).panel('options');
					if (opt.closable && type === 'closeOther' && opt.title != curTabTitle) {
						closeTabsTitle.push(opt.title);
					} else if (opt.closable && type === 'closeAll') {
						closeTabsTitle.push(opt.title);
					} else if (opt.closable && type == 'closeRight'){
						if(tabIndex < index_tabs.tabs('getTabIndex', $(this))){
							closeTabsTitle.push(opt.title);
						}
...
<body>
	<jsp:include page="sys/userLogin.jsp"></jsp:include>
	<jsp:include page="sys/userReg.jsp"></jsp:include>
	<div id="index_layout">
		<div data-options="region:'north',href:'${pageContext.request.contextPath}/layout/north.jsp'" style="height:45px; overflow: hidden;" class="logo"></div>
 ...
 <%-- 	<div data-options="region:'east',href:'${pageContext.request.contextPath}/layout/east.jsp'" title="日历" style="width: 230px; overflow: hidden;"></div>	 --%>
	...
	<div id="index_tabsMenu" style="width: 120px; display: none;">
		...
		<div title="closeRight" data-options="iconCls:'delete'">关闭右侧</div>
		<div title="closeLeft" data-options="iconCls:'delete'">关闭左侧</div>
		<div title="closeAll" data-options="iconCls:'delete'">关闭所有</div>
		<div title="moveToLeft" data-options="iconCls:'delete'">左移</div>
		<div title="moveToRight" data-options="iconCls:'delete'">右移</div>
	</div>
