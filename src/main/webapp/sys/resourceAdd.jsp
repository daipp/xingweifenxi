		...
		<form id="form" method="post">
			<table class="table table-hover table-condensed">
				<tr>
					...
					<th>资源名称</th>
					<td><input name="resourceName" type="text" placeholder="请输入资源名称" class="easyui-validatebox span2" data-options="required:true" value=""></td>
				</tr>
				<tr>
					<th>资源路径</th>
					...
					<td><select name="typeId" class="easyui-combobox" data-options="width:140,height:29,editable:false,panelHeight:'auto'">
							<c:forEach items="${resourceTypeList}" var="resourceType">
								<option value="${resourceType.id}">${resourceType.codeContent}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr>
					...
					<th>上级资源</th>
					<td>
						<select id="pid" name="pid" style="width: 140px; height: 29px;"></select>
						<img src="${pageContext.request.contextPath}/style/images/extjs_icons/cut_red.png" onclick="$('#pid').combotree('clear');" />
					</td>
				</tr>
				<tr>
					<th>菜单图标</th>
					<td><input id="icon" name="icon" style="width: 140px; height: 29px;" data-options="editable:true" /></td>
					<th>是否展开</th>
					<td>
					...
					<td colspan="3">
						<select name="topMenuId" class="easyui-combobox" data-options="width:140,height:29,editable:false,panelHeight:'auto'">
							<c:forEach items="${topMenuList}" var="tm">
								<option value="${tm.id}">${tm.codeContent}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
