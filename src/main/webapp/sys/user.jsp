...
<c:if test="${fn:contains(sessionInfo.resourceList, '/userControoler/bossCode')}">
	<script type="text/javascript">
		$.canBOSSCode = true;
	</script>
</c:if>
<script type="text/javascript">
		...
		dataGrid = $('#dataGrid').datagrid({
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					...
					str += '&nbsp;';
					if ($.canBOSSCode) {
						str += $.formatString('<img onclick="editBossSysCodeFun(\'{0}\');" src="{1}" title="修改BOSS代码关联"/>', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/repot/report.png');
					}
					return str;
				}
			} ] ],
	...
	function editBossSysCodeFun(id) {
		if (id == undefined) {
			var rows = dataGrid.datagrid('getSelections');
			id = rows[0].id;
		} else {
		...
		parent.$.modalDialog({
			title : '修改用户与BOSS系统代码关联',
			width : 600,
			height : 600,
			href : '${pageContext.request.contextPath}/userController/bossCode?id=' + id,
			buttons : [ {
				text : '重置',
				handler : function() {
...
<!-- 	<div class="easyui-layout" data-options="fit : true,border : false">
		<div data-options="region:'north',title:'查询条件',border:false" style="overflow: hidden;"> 
-->
	<div id="queryPanels" class="easyui-layout" data-options="fit:true,border:false,title:'查询条件'" style="min-width:1000px">
		<div style="overflow: hidden; height:105px" 
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="display: none;">
					<tr>
						...
						<td colspan=3><input name="loginName" placeholder="可以模糊查询登录名" class="span2" /></td>
					</tr>
					<tr>
						<th style="width:80px">创建日期</th>
						<td style="width:300px"><input class="span2" name="createdatetimeStart" placeholder="点击选择时间" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" />至<input class="span2" name="createdatetimeEnd" placeholder="点击选择时间" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" /></td>
						<th style="width:80px">最后修改日期</th>
						<td><input class="span2" name="modifydatetimeStart" placeholder="点击选择时间" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" />至<input class="span2" name="modifydatetimeEnd" placeholder="点击选择时间" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" /></td>
					</tr>
				</table>
			</form>
		</div>
