...
var gridTooltipOptionsHead = {
	//显示列头tip
	showHeadTip : function(jq) {
		return jq.each(function() {
			var panel = $(this).datagrid('getPanel');
			var columns = $(this).datagrid('options').columns[0];
			for(var i=0;i<columns.length;i++){
				var col = columns[i];
				if(col.headTip && col.headTip!=''){
					showTooltip($(".datagrid-header td[field='" + col.field +  "']", panel),col.headTip);
				}
			}
		});

		function showTooltip(jqs,txt) {
			jqs.mouseover(function () {
				var tipContent = txt;
				if(tipContent == null || '' == tipContent){
					tipContent = $(this).text();
				}
				$(this).tooltip({
					position: 'top',
					content : tipContent,
...
$.extend($.fn.datagrid.methods, gridTooltipOptionsHead);
...
$.fn.tree.defaults.loadFilter = function(data, parent) {
	...
	if (opt.parentField) {
		...
		//第二次遍历：state='colosed'的且无子节点的节点,把state改为'open'(否则测试发现会出现无限嵌套);
		//顶级节点塞入treeData			
		for (i = 0, l = data.length; i < l; i++) {
			if(data[i]['state'] && data[i]['state'] == 'closed') {
				if (!data[i]['children']){
					data[i]['state'] = 'open';
				}
			}
