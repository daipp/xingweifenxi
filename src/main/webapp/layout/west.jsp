...
<script type="text/javascript">
	...
	$(function() {
		layout_west_tree = $('#layout_west_tree').tree({
			url : layout_west_tree_url+"?topMenuId=1003",
			parentField : 'pid',
			//lines : true,
			...
			onBeforeLoad : function(node, param) {
				if (layout_west_tree_url+"?topMenuId=1003") { //只有刷新页面才会执行这个方法
					parent.$.messager.progress({
						title : '提示',
						text : '数据处理中，请稍后....'
					});
		...
		layout_west_tree = $('#layout_west_tree2').tree({
			url : layout_west_tree_url+"?topMenuId=1005",
			parentField : 'pid',
			//lines : true,
			onClick : function(node) {
				if (node.attributes && node.attributes.url) {
			...
			onBeforeLoad : function(node, param) {
				if (layout_west_tree_url+"?topMenuId=1005") {//只有刷新页面才会执行这个方法
					parent.$.messager.progress({
						title : '提示',
						text : '数据处理中，请稍后....'
					});
...
<div class="easyui-accordion" data-options="fit:true,border:false">
	...
	<div title="系统菜单" style="padding: 5px;" data-options="border:false,iconCls:'anchor',tools : [ {
	...
	<div title="统计分析报表" style="padding: 5px;" data-options="border:false,iconCls:'anchor',selected:true,tools : [ {
				iconCls : 'database_refresh',
				handler : function() {
				...
				handler : function() {
					...
					if (node) {
						$('#layout_west_tree2').tree('expandAll', node.target);
					} else {
						$('#layout_west_tree2').tree('expandAll');
					}
				}
			}, {
				iconCls : 'resultset_previous',
				handler : function() {
					var node = $('#layout_west_tree2').tree('getSelected');
					if (node) {
						$('#layout_west_tree2').tree('collapseAll', node.target);
					} else {
						$('#layout_west_tree2').tree('collapseAll');
					}
				}
			} ]">
